...
export default function YellowPoints({ navigation, route }) {
    ...
    useEffect(() => {
        db.transaction(tx => {
            tx.executeSql('select * from yellowpoints order by id desc;', [], (_, { rows }) =>
    ...
    const saveTown = () => {
        db.transaction(tx => {
            tx.executeSql('insert into yellowpoints (feature, score, color) values (?, ?, ?);',
                ['Kaupunki', parseInt(scoreTown * 2), 'yellow']);
        }, null, updateList
        )
    }
    ...
    const saveInn = () => {
        db.transaction(tx => {
            tx.executeSql('insert into yellowpoints (feature, score, color) values (?, ?, ?);',
                ['Kievaritie', parseInt(scoreInn * 2), 'yellow']);
        }, null, updateList
        )
    ...
    const saveMonastery = () => {
        db.transaction(tx => {
            tx.executeSql('insert into yellowpoints (feature, score, color) values (?, ?, ?);',
                ['Luostari', parseInt(scoreMonastery), 'yellow']);
        }, null, updateList
        )
    }
    ...
    const saveCathedral = () => {
        db.transaction(tx => {
            tx.executeSql('insert into yellowpoints (feature, score, color) values (?, ?, ?);',
                ['Suurkatedraali', parseInt(scoreCathedral * 3), 'yellow']);
        }, null, updateList
        )
    ...
    const deleteItem = (id) => {
        db.transaction(
            tx => {
                tx.executeSql(`delete from yellowpoints where id = ?;`, [id]);
            }, null, updateList
        )
    }
        ...
        <ImageBackground
            ...
            <View style={styles.pointcontainer}>
                <View style={styles.featureWrapper}>
                    <Text style={styles.pointsheader}>Keltainen</Text>
